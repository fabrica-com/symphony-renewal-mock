...
const initialMessages: ChatMessage[] = [
  {
    ...
    content:
      ...
      "【稼働中エージェント: 12体】\n" +
      "- AA相場ウォッチャー: USS東京をスキャン中\n" +
      "- 在庫価格自動調整: 142台の価格分析完了\n" +
      "- 問い合わせ自動返信: LINE 5件に返信済み\n" +
      "- 顧客フォローアップ: 納車7日後フォロー送信中\n" +
      "- 口コミ巡回: ネガティブ口コミ検出（要返信）\n\n" +
      "【承認キュー: 7件】\n" +
      "緊急: USS東京 BMW 320i 入札（45分後）、ネガティブ口コミ返信\n\n" +
      "車両・顧客の新規登録は各タブの「データ取込」から生素材を投入してください。",
  ...
  if (message.includes("レポート") || message.includes("売上") || message.includes("今日")) {
    return {
      content:
        ...
        "- 佐藤様（アルファード）: 72% / 598万円\n\n" +
        "【エージェント稼働状況（12体）】\n" +
        "- 仕入れ: AA相場ウォッチャー / 買取査定サポート\n" +
        "- 在庫: 価格自動調整 / 滞留アラート\n" +
        "- 集客: SNS投稿 / 競合監視\n" +
        "- 接客: 問い合わせ返信 / 商談サポート\n" +
        "- アフター: フォローアップ / 口コミ巡回 / 車検通知\n\n" +
        "本日の処理: 158件 / トークン消費: 33% / 自動処理率: 94.2%",
    }
  }
